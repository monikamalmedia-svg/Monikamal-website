...
Hashes: `#portfolio`, `#pipeline`, `#pricing`, `#faq`. About → `/about`. Right (`md+`, `hidden md:flex`): LanguageSwitcher + **Let’s Talk** / **Aanvraag** (`Navbar.contactCta`, ghost `border-glass-border` + `bg-black/30`, opens Get in touch modal). Mobile (`md:hidden`): same CTA (scroll `#contact`) + ghost burger — not the desktop cluster. Overlay: section links + LanguageSwitcher only. Overlay `md:hidden`. Hero **Book a project** → `#pricing`; **View work** → `#portfolio`. Nav `z-50`.
